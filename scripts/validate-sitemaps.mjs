...
const SITE = 'https://cheatsforvalorant.com';
...
async function main() {
	...
	const valorantCheats = path.join(DIST, 'valorant-cheats', 'index.html');
	try {
		const cheatsStat = await stat(valorantCheats);
		if (cheatsStat.size < 2000) {
			fail(`/valorant-cheats/ built as redirect stub (${cheatsStat.size} bytes)`);
			bump();
		} else {
			ok(`/valorant-cheats/ builds as live pillar page (${cheatsStat.size} bytes)`);
		}
	} catch {
		fail('Missing /valorant-cheats/index.html — pillar page not built');
		bump();
	}
